...
/**
 * This function will verify that the auth token is valid
 *
 */
async function decodeAuth(token: string) {
  try {
    const userInfo = await firebaseAuth.verifyIdToken(token);
    return userInfo;
  } catch (e) {
    throw AuthError.ERROR_DECODING_TOKEN;
  }
}

export { createUser, decodeAuth };
